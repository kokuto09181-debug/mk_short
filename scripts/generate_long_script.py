...
def generate_script(client: LLMClient, figure: dict) -> str:
    """LLMで日本語脚本を生成してプレーンテキストを返す"""
...
def run(limit: int = 20, force_all: bool = False, backend: str = ""):
    """メイン処理"""
    notion = NotionFigureClient()
    ai_client = LLMClient(backend=backend or None)
    logger.info(f"LLMバックエンド: {ai_client.backend}, モデル: {ai_client.model}")
    ...
    for i, figure in enumerate(figures, 1):
        ...
        try:
            script_text = generate_script(ai_client, figure)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--backend", type=str, default="",
                        ...
                        help="LLMバックエンド（省略時は settings.yaml の設定を使用）")
    args = parser.parse_args()

    run(limit=args.limit, force_all=args.force_all, backend=args.backend)
